...
`)();
  ...
  assert.deepStrictEqual(
    snapshot.messages.map(({ type }) => type),
    ['GET_STEP5_SUBMIT_STATE', 'COMPLETE_STEP5_CHATGPT_ONBOARDING']
  );
...
test('step 5 post-completion validation skips onboarding even when tab URL already reached chatgpt.com', async () => {
  const api = new Function(`
const logs = [];
const messages = [];
...
`)();
  ...
  assert.equal(
    snapshot.logs.some(({ message }) => /已进入 chatgpt\.com 且检测到 ChatGPT 用途选择页/.test(message)),
    true
  );
});
